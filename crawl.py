...
for i in bloglinklist:
    ...
    with open("out/"+path + '/' + filename, 'r') as txtfile, open('summary.csv', 'a', encoding='utf-8') as csvfileout:
        ...
        writer = csv.DictWriter(csvfileout, fieldnames=[
                                "num", "content", "img", "sticker", "like", "tags"])
        writer.writerow({'num': filename.rstrip('.txt'), 'content': line,
                         'img': imgCnt, 'sticker': stiCnt, 'like': strheart, 'tags': strtag})
